    ...
    <main className="flex flex-1 bg-muted/30 py-10 md:py-16">
      <Container>
        <div className="mx-auto w-full max-w-6xl">
          ...
          <section className="mt-10" aria-labelledby="vocabulary-list">
            <div className="mb-5 flex items-end justify-between gap-4">
              <h2 id="vocabulary-list" className="text-h2">{t("available")}</h2>
              <span className="text-body-sm text-muted-foreground">{data.items.length}</span>
            </div>
            {data.items.length ? (
                  ...
                  <Card key={item.id} className="group">
                    <CardHeader>
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <CardTitle className="font-en text-2xl">{item.word}</CardTitle>
                          {item.partOfSpeech ? <p className="text-caption mt-1 text-muted-foreground">{item.partOfSpeech}</p> : null}
                        </div>
                        <ProgressBadge domain="vocabulary" state={item.state} />
                      </div>
                      <p className="text-body-sm mt-3 text-muted-foreground">{localize(item.translation, locale)}</p>
                    </CardHeader>
                    <CardContent>
                      <Link href={`/vocabulary/${item.id}`} className="inline-flex items-center gap-2 text-body-sm font-medium text-primary hover:underline">
